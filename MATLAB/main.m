...
%% ------------------ LYAUPUNOV EXPONENT CALCULATION ------------------

fprintf('\nCalculating Lyapunov Exponents (LEs)...\n');

% Initialize system state as double
X = double([X_Initial; Y_Initial; Z_Initial; W_Initial]);

% Initialize orthogonal perturbation matrix
V = eye(4, 'double');
LE = zeros(4, 1);
LE_track = zeros(4, floor(N/10));

for n = 1:N
    x = X(1); y = X(2); z = X(3); w = X(4);

    % Use your F, G, R, S functions
    dx = F(x, y, w, alpha);
    dy = G(x, y, z, gamma, epsilon);
    dz = R(x, y, z, beta);
    dw = S(y, z, w, lambda);
    ...
    if any(~isfinite(X))
       warning('Numerical overflow at step %d', n);
       break;
    end

    % Double-precision Jacobian matrix
    J = double([-alpha, alpha, 0, 1;
                gamma - z, epsilon, -x, 0;
                y, x, -beta, 0;
                0, z, y, lambda]);

    % Propagate perturbation matrix
    V = V + h * (J * V);

    % QR decomposition every 10 steps
    if mod(n,10)==0
        [Q, r] = qr(V);
        V = Q;
        Rdiag = abs(diag(r));
       Rdiag(~isfinite(Rdiag) | Rdiag<=0) = eps; % avoid log(0)
        LE = LE + log(Rdiag);
        LE_track(:, n/10) = LE / (n*h);
    end
end

LE_final = LE / (N*h);

fprintf('\nLyapunov Exponents (LE1 to LE4):\n');
disp(LE_final);

%% Plot LEs vs Steps
steps_vec = linspace(0, N*h, size(LE_track, 2));
figure;
plot(steps_vec, LE_track(1,:), 'b', 'LineWidth', 1.5); hold on;
plot(steps_vec, LE_track(2,:), 'r', 'LineWidth', 1.5);
plot(steps_vec, LE_track(3,:), 'color', [1 0.5 0], 'LineWidth', 1.5);
plot(steps_vec, LE_track(4,:), 'm', 'LineWidth', 1.5);
grid on;
xlabel('Steps'); ylabel('LEs');
title('The four Lyapunov exponents (LEs)');
legend('LE1','LE2','LE3','LE4', 'Location','best');

text(25, LE_final(1), sprintf('LE1 = %.4f', LE_final(1)), 'Color','b', 'FontWeight','bold');
text(15, LE_final(2), sprintf('LE2 = %.4f', LE_final(2)), 'Color','r', 'FontWeight','bold');
text(30, LE_final(3), sprintf('LE3 = %.4f', LE_final(3)), 'Color',[1 0.5 0], 'FontWeight','bold');
text(20, LE_final(4), sprintf('LE4 = %.4f', LE_final(4)), 'Color','m', 'FontWeight','bold');
...
%% HISTOGRAM ANALYSIS
% Compute histogram variance and standard deviation
...
for k = 1:numel(crop_levels)
    ...
    cipher_cropped =temp ;
    corrupted_ciphers{k} = cipher_cropped;
...
alpha = 0.11;  % diffusion factor (reduces excessive randomness)
beta = 0.15;   % local averaging weight
...
% === Combined Figure (like paper Figure 13) ===
...
%% === SALT AND PEPPER ATTACK ( Figure 14) ===
fprintf('\n=== SALT AND PEPPER (Reproduced Figure 14) ===\n');
...
% === Combined Figure ( Figure 14) ===
figure('Name','Salt and Pepper Attack (Reproduced Figure 13)','Position',[80 80 1400 700]);
...
%  %% --- Verify ---
%  if isequal(img, dec_img)
%      disp('? Success: Decrypted image matches the original.');
%  else
%      disp('? Error: Decrypted image does not match the original.');
%  end
